        ...
        StateMachine & aStateMachine)
{
    ...
    for (const auto command : mSelfPopCommands)
    {
        if (isPositiveEdge(aInputs, command) != Controller::_End)
        {
            aStateMachine.popState();
        }
    }

    const Controller pressSelectController = isPositiveEdge(aInputs, Jump);
    ...
    else if (pressSelectController != Controller::_End)
    {
        mMenu.selected().mCallback(aStateMachine, mAppInterface, pressSelectController);
...
MenuScene & MenuScene::addSelfPopCommand(Command aCommand)
{
    mSelfPopCommands.push_back(aCommand);
    return *this;
}
